...
### Текущая реализация (декабрь 2025)
- `useCouncilPlan` — выделенный хук в `src/pages/workspace/model` который строит план, управляет волнами и держит `AbortController`. `useWorkspaceModel` только пробрасывает состояние и экшены.
- `councilPlanTypes.ts` — единая типизация `CouncilPlan`, `CouncilBranch`, `BranchStatus`, используемая Canvas/ChatPanel/моделью.
- План волнами обновляется пошагово: каждый wave запускает параллельные ветки через `playCouncil`, статусы (`queued → running → done/error`) транслируются в чат-панель.
- В DEV окружении включён логгер `[CouncilPlan] ...` для диагностики параллелизма и времени волн; лог отключён в проде.
